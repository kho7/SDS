...
# ------------------------------------------
# Step 2: Clean the data.  Replace missing values with mean.
...
#or air$Date <- as.Date(paste("1973", airquality$Month, airquality$Day, sep="-"))
...
# Set your own color choice
ggplot(air, aes(x=Wind, y=Temp)) + geom_point(aes(size = Ozone, color=Solar.R)) + 
   scale_colour_gradientn(colours = terrain.colors(10))

ggplot(air, aes(x=Wind, y=Temp)) + geom_point(aes(size = Ozone, color=Solar.R)) + 
  scale_colour_gradientn(colours = heat.colors(20))

ggplot(air, aes(x=Wind, y=Temp)) + geom_point(aes(size = Ozone, color=Solar.R)) + 
  scale_color_distiller(palette = "RdPu")

# Try different color 
# e.g. + scale_color_distiller(palette = "RdPu")
# other choices: Blues BuGn BuPu GnBu Greens Greys Oranges OrRd PuBu PuBuGn 
