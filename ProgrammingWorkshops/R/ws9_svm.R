# SDS: Workshop
# Topic: Machine learning usign support vector machines (SVM)
# (Adapted from Saltz and Stanton Introduction to Data Science)
...
# In order to split data, create a 2/3 cutpoint and round the number (floor function)
...
# arrange the three plot in one row
lay <- rbind(c(1,2,3,3))
grid.arrange(plot_svm,plot_svm_e,plot_lm,layout_matrix = lay)
# alternatively arrange the three plots in 2x2 matrix
...
ggplot(Plot_ksvm, aes(x=Temp,y=Wind)) + 
   geom_point(aes(color=correct,size=goodOzone,shape = Predict))
...
# plot result using ggplot
ggplot(Plot_svm_e,aes(x=Temp,y=Wind)) + geom_point(aes(color=correct,size=goodOzone,shape=Predict))
...
# plot result using ggplot
ggplot(Plot_nb,aes(x=Temp,y=Wind)) + geom_point(aes(color=correct,size=goodOzone,shape=Predict))

# show all charts in one window
g1 <- ggplot(Plot_ksvm,aes(x=Temp,y=Wind))+geom_point(aes(color=correct,size=goodOzone,shape = Predict)) + theme(legend.position="none") + ggtitle("ksvm")
g2 <- ggplot(Plot_svm_e,aes(x=Temp,y=Wind))+geom_point(aes(color=correct,size=goodOzone,shape=Predict)) + theme(legend.position="none") + ggtitle("svm")
g3 <- ggplot(Plot_nb,aes(x=Temp,y=Wind))+geom_point(aes(color=correct,size=goodOzone,shape=Predict)) + theme(legend.position="right") + ggtitle("Naive Bayes")
lay <- rbind(c(1,2,3))
grid.arrange(g1,g2,g3,layout_matrix = lay)

# --------------------------------------------------------------------
# Step 6: Which are the best Models for this data? 
# When using the real Ozone values to build model, the RMSE for svm is the smallest, while linear model has relatively high value and ksvm has the highest value for RMSE. So in this situation ksvm is the best model for this data.
# When using goodOzone to predict the testdata, The percentage of "goodOzone" predicted correctly was calculated. It turns out the naive bayes has the highest correct prediction score, while ksvm has the lowest.
